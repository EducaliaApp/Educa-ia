-- ============================================
-- PASO 2: Crear tabla function_logs (CORREGIDO)
-- ============================================

-- Crear tabla function_logs si no existe
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'function_logs_level_check'
    ) THEN
        ALTER TABLE function_logs
        ADD CONSTRAINT function_logs_level_check
        CHECK (level IN ('debug', 'info', 'warn', 'error'));
        RAISE NOTICE 'Constraint function_logs_level_check creada';
    ELSE
        RAISE NOTICE 'Constraint function_logs_level_check ya existe';
    END IF;
END $$;

-- Índices para consultas rápidas (crear solo si no existen)
...
DROP POLICY IF EXISTS "Administradores pueden ver todos los logs" ON function_logs;
DROP POLICY IF EXISTS "Service role puede insertar logs" ON function_logs;
...
-- Verificación
SELECT
    'function_logs' as tabla,
    COUNT(*) as total_constraints
FROM pg_constraint
WHERE conrelid = 'function_logs'::regclass;
