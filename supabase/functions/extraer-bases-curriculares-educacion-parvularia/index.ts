/**
 * Edge Function para extraer Bases Curriculares de Educación Parvularia
 *
 * Esta función extrae específicamente los datos de:
 * https://www.curriculumnacional.cl/curriculum/educacion-parvularia
 *
 * Ventajas de la función específica:
 * - Mejor control del timeout (función más corta)
...
const CATEGORY_URL = 'https://www.curriculumnacional.cl/curriculum/educacion-parvularia'
const CATEGORY_NAME = 'Educación Parvularia'
const FUNCTION_NAME = 'extraer_bases_curriculares_educacion_parvularia'
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Extraer objetivos de la categoría
    ...
    const archivosGenerados = await generarYSubirArchivos(
      ...
      resultado.categoria,
      generate_files,
      resultado.asignaturasProcesadas
    )
    ...
    return new Response(
      JSON.stringify({
        error: `Error en extracción de bases curriculares - ${CATEGORY_NAME}`,
